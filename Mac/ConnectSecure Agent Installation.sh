...
cybercnscompany_id=$(/Applications/NinjaRMMAgent/programdata/ninjarmm-cli get connectsecureCompanyId)
...
function install_lightweight {
   ...
   pgrep cybercnsagent_darwin
   exitcode=$?
   if [ $exitcode -eq 0 ]; then
      echo "lightweight agent already running in the machine"
   else
      echo "Downloading Cybercns Agent"
      download_url=$(curl -s -L https://configuration.myconnectsecure.com/api/v4/configuration/agentlink?ostype=darwin)
      exitcode=$(curl --write-out %{http_code} -L -g "$download_url" -s -o cybercnsagent_darwin)
      ...
      sudo ./cybercnsagent_darwin -c $cybercnscompany_id -e $cybercnstenantid -j $cybercnstoken -i;
   fi
}
